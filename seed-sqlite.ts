...
if (!process.env.SMDT_DATA_DIR) {
  console.error(
    "SMDT_DATA_DIR is required. Example: SMDT_DATA_DIR=../smdt-legacy/backend/data npm run db:seed",
  );
  process.exit(1);
}
...
async function main() {
  ...
  const aggCount = db.prepare(`SELECT COUNT(*) AS cnt FROM station_heatmap`).get() as {
    cnt: number;
  };
