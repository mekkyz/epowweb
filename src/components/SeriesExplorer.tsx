...
export default function SeriesExplorer({ type, id }: Props) {
  ...
  const download = async (format: "json" | "csv") => {
    try {
      ...
      const raw = await res.json();
      const series = raw?.data?.series ?? raw?.series ?? raw?.data ?? [];
      ...
      <div className="mb-4">
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div className="flex items-end gap-2">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-foreground-secondary flex items-center gap-1.5 text-xs font-medium tracking-wider uppercase">
          ...
          {/* Expand/Shrink Range */}
          <div className="border-border flex items-end gap-1 border-l pl-3">
            <Button
              variant="outline"
              size="sm"
            ...
            <Select
              ...
              disabled={isDemo}
              size="sm"
              className="border-border-strong dark:bg-background bg-white py-1"
            />
            <Button
              variant="outline"
              size="sm"
        ...
        {bounds.start && bounds.end && (
          <div className="border-border mt-3 flex items-center gap-2 border-t pt-3">
            <span className="text-foreground-tertiary text-xs font-medium">Available range:</span>
            <span className="bg-surface text-foreground-secondary rounded-md px-2 py-0.5 font-mono text-xs">
