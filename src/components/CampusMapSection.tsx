...
const viewOptions = [
  {
    ...
    label: (
      <>
        <Layers className="h-4 w-4" /> 2D View
      </>
    ),
  },
  {
  ...
  return (
    <section id="map" className="mt-6 scroll-mt-20 space-y-4">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <PageHeader label={config.label} title={config.title} className="mb-0" />
      ...
      <div className="bg-panel border-border text-foreground-secondary flex items-center gap-2 rounded-lg border px-4 py-3 text-sm">
        <Sparkles className="h-4 w-4 flex-shrink-0 text-emerald-400" />
        <p>
          <strong className="text-foreground">Select a station</strong> for details. Use{" "}
          <strong className="text-foreground">ALT + drag</strong> for view rotation.
        </p>
      </div>
    </section>
  );
