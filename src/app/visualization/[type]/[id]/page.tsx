...
export default async function VisualizationPage({ params, searchParams }: Params) {
  ...
  const fallbackLabel =
    station?.properties.id || building?.id || meter?.id || id || safeType.toUpperCase();
  const subtitle =
  ...
  return (
    <div className={`mx-auto max-w-6xl px-4 py-8 ${embed ? "bg-panel min-h-screen" : ""}`}>
      {!embed && <VisualizationHeader type={safeType} title={fallbackLabel} subtitle={subtitle} />}
